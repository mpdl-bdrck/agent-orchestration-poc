...
# Examples:
#   ./run_campaign_portfolio_pacing.sh                                    # Account 17, Eli Lilly campaigns (defaults)
#   ./run_campaign_portfolio_pacing.sh 17                                 # Account 17, Eli Lilly campaigns
#   ./run_campaign_portfolio_pacing.sh 17 "Lilly"                        # Account 17, Eli Lilly campaigns (explicit)
#   ./run_campaign_portfolio_pacing.sh 17 "Lilly" --publish-sheets       # + Publish comprehensive rollups to Google Sheets
#   ./run_campaign_portfolio_pacing.sh 17 "Lilly" --publish-sheets --campaign-start 2025-11-01 --campaign-end 2025-12-31 --campaign-budget 466000  # + Create pacing dashboard
...
#   --publish-sheets           # Publish comprehensive rollups to Google Sheets (6 worksheets)
...
#
# Default: ./run_campaign_portfolio_pacing.sh (uses account 17 with Eli Lilly, Nov 1st to today, no Sheets publishing)
...
if [ "$PUBLISH_SHEETS" = true ]; then
    echo "📊 Google Sheets publishing: ENABLED (6 worksheets)"
...
if [ "$CREATE_SUMMARY" = true ] && [ "$PUBLISH_SHEETS" = false ]; then
    print_error "--create-summary requires --publish-sheets to be enabled"
    exit 1
fi
