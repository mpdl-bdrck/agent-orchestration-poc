#!/usr/bin/env python3
"""
Deal Debugger - Enhanced with Diagnostic Engine
================================================

Debugs BidSwitch deal performance with comprehensive diagnostics.
...
class DealDebugger:
    """Enhanced deal debugger with comprehensive diagnostics"""
    ...
    def run_analysis(self) -> Dict[str, Any]:
        """
        Run comprehensive deal debugging with enhanced diagnostics
        """
        print(f"🔍 Enhanced BidSwitch Deal Debugging for Deal {self.deal_id}")
        print("=" * 70)
        ...
        try:
            ...
            print(f"❌ Debugging failed for deal {self.deal_id}: {e}")
            results = {
                'deal_id': self.deal_id,
                'analysis_timestamp': datetime.now().isoformat(),
                'error': str(e),
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description='Enhanced BidSwitch Deal Debugging Tool')
    parser.add_argument('deal_id', help='Deal ID to debug')

    args = parser.parse_args()

    try:
        debugger = DealDebugger(args.deal_id)
        results = debugger.run_analysis()

        if results['status'] == 'SUCCESS':
            diagnosis = results['diagnosis']
            print(f"\n✅ Enhanced deal debugging completed!")
            print(f"🎯 Primary Issue: {diagnosis['primary_issue']} ({diagnosis['severity']})")
            print(f"🎯 Confidence: {diagnosis['confidence']:.0%}")
            return 0
        else:
            print(f"\n❌ Enhanced deal debugging failed!")
            return 1
