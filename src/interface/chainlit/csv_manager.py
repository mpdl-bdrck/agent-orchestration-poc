...
def retrieve_csv_from_session(node_name: str = None) -> tuple[str | None, str | None]:
    ...
    logger.info(f"🔍 Checking session CSV attachments: {list(csv_attachments.keys())}")
    ...
    for tool_name, csv_info in csv_attachments.items():
        if node_name is None or csv_info.get("node_name") == node_name:
            ...
            if csv_data and csv_filename:
                logger.info(f"✅ Found CSV in session storage: filename={csv_filename}")
...
def retrieve_csv_from_file_cache(node_name: str = None) -> tuple[str | None, str | None]:
    ...
    try:
        ...
        logger.info(f"🔍 File cache check: found {len(cache_files)} cache files")
        ...
        for cache_file in cache_files:
            try:
                with open(cache_file, 'r') as f:
                    ...
                    csv_filename = csv_info.get("filename")
                    if csv_data and csv_filename:
                        logger.info(f"✅ Found CSV in file cache: {cache_file}, filename={csv_filename}")
...
def retrieve_csv_from_global_storage(node_name: str = None) -> tuple[str | None, str | None]:
    ...
    try:
        ...
        logger.error(f"❌ Error accessing global storage: {e}", exc_info=True)
        return None, None
...
def retrieve_csv_from_module_storage(node_name: str = None) -> tuple[str | None, str | None]:
    ...
    try:
        ...
        logger.info(f"🔍 Searching all modules for execute_agent_loop with _csv_storage...")
        modules_checked = 0
                ...
                if hasattr(mod, 'execute_agent_loop'):
                    ...
                    if hasattr(candidate_func, '_csv_storage'):
                        ...
                        if isinstance(storage_dict, dict) and len(storage_dict) > 0:
                            ...
                            logger.info(f"✅ Found execute_agent_loop with _csv_storage in module: {mod_name}, keys: {list(storage_dict.keys())}")
                            break
            except Exception as e:
                logger.debug(f"Error checking module {mod_name}: {e}")
                continue
        
        logger.info(f"🔍 Searched {modules_checked} modules")
        ...
        if not execute_agent_loop and module_name in sys.modules:
            ...
            if hasattr(agent_loop_module, 'execute_agent_loop'):
                ...
                if hasattr(candidate_func, '_csv_storage'):
                    execute_agent_loop = candidate_func
                    logger.info(f"✅ Found execute_agent_loop in sys.modules!")
        ...
        if execute_agent_loop and hasattr(execute_agent_loop, '_csv_storage'):
            ...
            logger.info(f"✅ CSV storage exists! Keys: {storage_keys}")
            ...
            for storage_key, csv_info in execute_agent_loop._csv_storage.items():
                if node_name == "guardian" and "analyze_portfolio_pacing" in storage_key:
                    ...
                    csv_filename = csv_info.get("filename")
                    if csv_data and csv_filename:
                        logger.info(f"✅ Found CSV in module storage! filename={csv_filename}, csv_len={len(csv_data)}")
        ...
        logger.error(f"❌ Error accessing module storage: {e}", exc_info=True)
        return None, None
...
def retrieve_csv_all_methods(node_name: str = None) -> tuple[str | None, str | None]:
    ...
    Returns:
        Tuple of (csv_data, csv_filename) or (None, None) if not found
    """
    logger.info(f"🔍 Starting CSV retrieval for node: {node_name}")
    ...
    logger.info(f"🔍 No CSV found using any method")
    return None, None
...
async def send_csv_as_message(csv_data: str, csv_filename: str) -> None:
    ...
    try:
        ...
        logger.info(f"🔍 Creating CSV file element: filename={csv_filename}, size={len(csv_bytes)} bytes")
        ...
        logger.info(f"🔍 CSV saved to temp file: {tmp_path}")
        ...
        try:
            ...
            logger.info(f"🔍 CSV element created with path: {csv_element}")
        except (TypeError, AttributeError, Exception) as e:
            # Fallback to content if path doesn't work
            logger.warning(f"🔍 Path failed ({e}), trying content instead...")
            csv_element = cl.File(
                name=csv_filename,
                content=csv_bytes,
                display="inline"
            )
            logger.info(f"🔍 CSV element created with content: {csv_element}")
        ...
        csv_message = cl.Message(
            content=f"📊 **Download:** {csv_filename}",
            elements=[csv_element],
            author="System"
        )
        
        logger.info(f"🔍 CSV message created, sending...")
        await csv_message.send()
        logger.info(f"✅✅✅ CSV file sent as new message: {csv_filename} (message ID: {getattr(csv_message, 'id', 'unknown')})")
