...
def create_specialist_node(call_specialist_agent_func, embedding_model, get_agent_func, streaming_callback=None):
    ...
    def specialist_node(state: AgentState) -> AgentState:
        ...
        try:
            ...
            # NOTE: Do NOT emit agent_response here - the Specialist agent already emits
            ...
            # Adding to messages causes duplicate display (Specialist response + Orchestrator echo)
            return {
                # DO NOT add to messages - only add to agent_responses
                # The Guardian node follows this pattern correctly - Specialist should match it
